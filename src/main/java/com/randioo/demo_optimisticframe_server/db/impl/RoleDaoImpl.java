package com.randioo.demo_optimisticframe_server.db.impl;
...
import com.randioo.demo_optimisticframe_server.db.converter.RoleConverter;
...
public class RoleDaoImpl extends DataAccess implements RoleDao {
	...
	public int insertRole(Role role, Connection conn) {
		int id = 0;
		final String sql = "insert into role(id,account,name,planesStr,usePlanesStr,score)"
		+ "values(?,?,?,?,?,?)";
		try {
			id = super.insertNotCloseConn(sql, new IntegerConverter(), conn, null, role.getAccount(), role.getName(),
					role.getPlanesStr(), role.getUsePlanesStr(), role.getScore());
	...
	public void updateRole(Role role) {
		final String sql = "update role set name=?,planesStr=?,usePlanesStr=?,score=?"
				+ " where id=? limit 1";
		try {
			Connection conn = dataSource.getConnection();
			super.update(sql, conn, role.getName(),role.getPlanesStr(),role.getUsePlanesStr(),role.getScore(), role.getRoleId());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
